...
const uint64 instr_kconfuzz_config = -5;
...
static void kconfuzz_wait_for_running_calls();
...
void execute_one()
{
	...
	for (;;) {
		...
		if (call_num == instr_kconfuzz_config) {
			uint64 param_id = read_input(&input_pos);
			uint64 value_id = read_input(&input_pos);
			uint64 flags = read_input(&input_pos);
			kconfuzz_wait_for_running_calls();
			kconfuzz_apply_config_action(param_id, value_id, flags);
			continue;
		}
	...
	close_fds();
#endif
	kconfuzz_restore_config_actions();
