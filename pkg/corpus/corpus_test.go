...
func TestCorpusIgnoresKConfuzzMetadata(t *testing.T) {
	target := getTarget(t, targets.TestOS, targets.TestArch64)
	corpus := NewCorpus(context.Background())
	rs := rand.NewSource(0)
	inp := generateInput(target, rs, 5)
	inp.KConfuzzConfig = &prog.KConfuzzConfigContext{
	...
	plain := inp
	plain.KConfuzzConfig = nil
	corpus.Save(plain)

	items := corpus.Items()
	assert.Len(t, items, 1)
	assert.Nil(t, items[0].KConfuzzConfig)
}
