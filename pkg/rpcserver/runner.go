...
func (runner *Runner) sendRequest(req *queue.Request) error {
	...
	case flatrpc.RequestTypeProgram:
		...
		actionSource := ""
		if !req.KConfuzzConfigDisabled {
			actions = req.KConfuzzConfigActions
			if len(actions) != 0 {
				actionSource = "request-actions"
				if req.KConfuzzConfigContext != nil {
					actionSource += "/context:" + req.KConfuzzConfigContext.Source
				}
			}
		}
		if len(actions) == 0 && !req.KConfuzzConfigDisabled && req.KConfuzzConfigContext != nil {
			actions = req.KConfuzzConfigContext.Actions
			if len(actions) != 0 {
				actionSource = "context:" + req.KConfuzzConfigContext.Source
			}
		}
		if len(actions) == 0 && !req.KConfuzzConfigDisabled {
			actions = kconfuzz.PlanFromEnv(req.Prog)
			if len(actions) != 0 {
				actionSource = "planner-env"
			}
		}
		if len(actions) != 0 {
			kconfuzz.Debugf("runner serializing %d config actions from %s for %d calls",
				len(actions), actionSource, len(req.Prog.Calls))
		}
		progData, err := req.Prog.SerializeForExecWithKConfuzz(actions)
		if err != nil {
