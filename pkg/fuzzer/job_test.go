...
func TestKConfuzzAnchorSignalIsDeterministic(t *testing.T) {
	anchor := kconfuzzAnchorSignal(signal.FromRaw([]uint64{0x30, 0x10, 0x20}, 0))
	assert.ElementsMatch(t, []uint64{0x10}, anchor.ToRaw())
}

func TestLimitKConfuzzActionsForSaveSpreadsSelection(t *testing.T) {
	var actions []prog.KConfuzzConfigAction
	for i := 0; i < 10; i++ {
		actions = append(actions, prog.KConfuzzConfigAction{ParamID: uint64(i)})
	}
	limited := limitKConfuzzActionsForSave(actions, 4)
	assert.Equal(t, []prog.KConfuzzConfigAction{
		{ParamID: 0},
		{ParamID: 2},
		{ParamID: 5},
		{ParamID: 7},
	}, limited)
	assert.Len(t, limitKConfuzzActionsForSave(actions, 20), len(actions))
}
