...
type Request struct {
	...
	// KConfuzzConfigActions are executor-only runtime parameter updates injected
	// into the exec buffer. KConfuzzConfigContext carries transient planning
	// metadata for the same actions. KConfuzzConfigDisabled is used by A/B
	// checks to force a plain execution even when a relation table is configured.
	KConfuzzConfigActions  []prog.KConfuzzConfigAction
	KConfuzzConfigContext  *prog.KConfuzzConfigContext
	KConfuzzConfigDisabled bool
...
func (r *Request) hash() hash.Sig {
	...
	if err := enc.Encode(r.KConfuzzConfigActions); err != nil {
		panic(err)
	}
	if err := enc.Encode(r.KConfuzzConfigContext.SignatureData()); err != nil {
		panic(err)
	}
	if err := enc.Encode(r.KConfuzzConfigDisabled); err != nil {
		panic(err)
	}
	var data []byte
	switch r.Type {
...
func (t *tee) Next() *Request {
	...
	t.queue.Submit(&Request{
		...
		Prog:                   req.Prog.Clone(),
		KConfuzzConfigActions:  append([]prog.KConfuzzConfigAction(nil), req.KConfuzzConfigActions...),
		KConfuzzConfigContext:  req.KConfuzzConfigContext.Clone(),
		KConfuzzConfigDisabled: req.KConfuzzConfigDisabled,
