...
func (target *Target) CalculatePriorities(corpus []*Prog, enabled map[*Syscall]bool) ([][]int32, map[*Syscall]bool) {
	return target.CalculatePrioritiesWithBoosts(corpus, enabled, nil)
}

type ChoiceTableBoost struct {
	From   *Syscall
	To     *Syscall
	Weight int32
}

func (target *Target) CalculatePrioritiesWithBoosts(corpus []*Prog, enabled map[*Syscall]bool,
	boosts []ChoiceTableBoost) ([][]int32, map[*Syscall]bool) {
	...
	if len(boosts) != 0 {
		boostPrio := target.calcBoostPrio(boosts, enabled)
		for i, prios := range boostPrio {
			dst := static[i]
			for j, p := range prios {
				dst[j] += p
			}
...
func (target *Target) calcBoostPrio(boosts []ChoiceTableBoost, enabled map[*Syscall]bool) [][]int32 {
	prios := make([][]int32, len(target.Syscalls))
	for i := range prios {
		prios[i] = make([]int32, len(target.Syscalls))
	}
	for _, boost := range boosts {
		if boost.From == nil || boost.To == nil {
			continue
		}
		if !enabled[boost.From] || !enabled[boost.To] {
			continue
		}
		weight := boost.Weight
		if weight <= 0 {
		...
		prios[boost.From.ID][boost.To.ID] += weight
	}
	normalizePrios(prios, len(enabled))
	return prios
}
...
func (target *Target) BuildChoiceTable(corpus []*Prog, enabled map[*Syscall]bool) *ChoiceTable {
	return target.BuildChoiceTableWithBoosts(corpus, enabled, nil)
}

func (target *Target) BuildChoiceTableWithBoosts(corpus []*Prog, enabled map[*Syscall]bool,
	boosts []ChoiceTableBoost) *ChoiceTable {
	prios, enabledCalls := target.CalculatePrioritiesWithBoosts(corpus, enabled, boosts)
