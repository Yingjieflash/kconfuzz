...
//  - execInstrKConfuzzConfig: applies an executor-only runtime configuration action
...
func (p *Prog) SerializeForExec() ([]byte, error) {
	return p.SerializeForExecWithKConfuzz(nil)
}

type KConfuzzConfigAction struct {
	BeforeCall int
	ParamID    uint64
	ValueID    uint64
	Flags      uint64
...
const (
	KConfuzzConfigFlagNoRestore = uint64(1 << iota)
	KConfuzzConfigFlagFlip
)

// SerializeForExecWithKConfuzz serializes p and injects executor-only runtime
// configuration actions before selected syscall indexes. The actions are not
// part of the syz program text/corpus.
func (p *Prog) SerializeForExecWithKConfuzz(actions []KConfuzzConfigAction) ([]byte, error) {
	p.debugValidate()
	w := &execContext{
		target: p.Target,
	...
	actionsByCall := make(map[int][]KConfuzzConfigAction)
	for _, action := range actions {
		if action.BeforeCall < 0 || action.BeforeCall > len(p.Calls) {
			return nil, fmt.Errorf("encodingexec: bad kconfuzz action call index %v/%v",
				action.BeforeCall, len(p.Calls))
		}
		actionsByCall[action.BeforeCall] = append(actionsByCall[action.BeforeCall], action)
	}
	for call, callActions := range actionsByCall {
		sort.SliceStable(callActions, func(i, j int) bool {
			if callActions[i].ParamID != callActions[j].ParamID {
				return callActions[i].ParamID < callActions[j].ParamID
			}
			return callActions[i].ValueID < callActions[j].ValueID
		})
		actionsByCall[call] = callActions
	}
	w.write(uint64(len(p.Calls)))
	for i, c := range p.Calls {
		w.writeKConfuzzConfigActions(actionsByCall[i])
	...
	w.writeKConfuzzConfigActions(actionsByCall[len(p.Calls)])
...
func (w *execContext) writeKConfuzzConfigActions(actions []KConfuzzConfigAction) {
	for _, action := range actions {
		w.write(execInstrKConfuzzConfig)
		w.write(action.ParamID)
		w.write(action.ValueID)
		w.write(action.Flags)
	}
}
