...
func TestSerializeForExecKConfuzzActions(t *testing.T) {
	target := initTargetTest(t, "test", "64")
	...
	actions := []KConfuzzConfigAction{
		{BeforeCall: 0, ParamID: 2, ValueID: 1, Flags: 0},
		{BeforeCall: 1, ParamID: 0, ValueID: 1, Flags: 1},
	}
	data, err := p.SerializeForExecWithKConfuzz(actions)
	if err != nil {
		t.Fatalf("failed to serialize with kconfuzz actions: %v", err)
	}
	decoded, err := target.DeserializeExec(data, nil)
	if err != nil {
		t.Fatal(err)
	...
	if len(decoded.Calls) != len(p.Calls) {
		t.Fatalf("decoded call count mismatch: %v/%v", len(decoded.Calls), len(p.Calls))
	}
	if !reflect.DeepEqual(decoded.KConfuzzActions, []ExecKConfuzzConfigAction{
		{BeforeCall: 0, ParamID: 2, ValueID: 1, Flags: 0},
		{BeforeCall: 1, ParamID: 0, ValueID: 1, Flags: 1},
	}) {
		t.Fatalf("decoded actions mismatch: %#v", decoded.KConfuzzActions)
	}
	if _, err := p.SerializeForExecWithKConfuzz([]KConfuzzConfigAction{{BeforeCall: len(p.Calls) + 1}}); err == nil {
		t.Fatalf("bad action call index was accepted")
	}
}
