...
func TestChoiceTableBoosts(t *testing.T) {
	target := initTargetTest(t, "linux", "amd64")
	from := target.SyscallMap["accept$inet"]
	to := target.SyscallMap["connect$inet"]
	if from == nil || to == nil {
		t.Fatalf("missing test syscalls: accept=%v connect=%v", from, to)
	}
	enabled := map[*Syscall]bool{
		from: true,
		to:   true,
	}
	base, _ := target.CalculatePriorities(nil, enabled)
	boosted, _ := target.CalculatePrioritiesWithBoosts(nil, enabled, []ChoiceTableBoost{{
		From:   from,
		To:     to,
		Weight: 3,
	}})
	if boosted[from.ID][to.ID] <= base[from.ID][to.ID] {
		t.Fatalf("boost did not increase priority %s -> %s: base=%d boosted=%d",
			from.Name, to.Name, base[from.ID][to.ID], boosted[from.ID][to.ID])
	}
	if boosted[to.ID][from.ID] != base[to.ID][from.ID] {
		t.Fatalf("one-way boost changed reverse priority %s -> %s: base=%d boosted=%d",
			to.Name, from.Name, base[to.ID][from.ID], boosted[to.ID][from.ID])
	}
}
