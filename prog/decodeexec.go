...
type ExecProg struct {
	Calls           []ExecCall
	Vars            []uint64
	KConfuzzActions []ExecKConfuzzConfigAction
}
...
type ExecKConfuzzConfigAction struct {
	BeforeCall uint64
	ParamID    uint64
	ValueID    uint64
	Flags      uint64
...
func (target *Target) DeserializeExec(exec []byte, stats map[string]int) (ExecProg, error) {
	...
	p := ExecProg{
		Calls:           dec.calls,
		Vars:            dec.vars,
		KConfuzzActions: dec.kconfuzzActions,
	}
	return p, nil
}
...
func (dec *execDecoder) parse() {
	...
	for dec.err == nil {
		...
		case execInstrKConfuzzConfig:
			dec.commitCall()
			dec.kconfuzzActions = append(dec.kconfuzzActions, ExecKConfuzzConfigAction{
				BeforeCall: uint64(len(dec.calls)),
				ParamID:    dec.read("instr/kconfuzz/param_id"),
				ValueID:    dec.read("instr/kconfuzz/value_id"),
				Flags:      dec.read("instr/kconfuzz/flags"),
			})
		default:
			dec.commitCall()
